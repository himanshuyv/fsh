...
int nextSubcommand(char** argv, Subcommand* ret, bool isBackground) {
    *ret = NULL;
    if (argv == NULL || argv[0] == NULL) {
        errorPrintf("Cannot pipe null command\n");
        return 0;
    }

    if (strcmp(argv[0], "|") == 0 || strcmp(argv[0], ">") == 0 || strcmp(argv[0], "<") == 0 || strcmp(argv[0], ">>") == 0) {
        errorPrintf("Cannot pipe empty command\n");
        return 0;
    }
    ...
    for (int i = argc; i < pipeSize; i++) {
        if (strcmp(argv[i], ">") == 0) {
            ...
            if (outCount > 1 || appendCount > 0) {
                errorPrintf("Cannot redirect to multiple files\n");
                return 0;
            }

            if (i + 1 >= pipeSize) {
                errorPrintf("Expected file after \'>\'\n");
                return 0;
            }
            ...
            inCount++;
            if (inCount > 1) {
                errorPrintf("Cannot redirect from multiple files\n");
                return 0;
            }
